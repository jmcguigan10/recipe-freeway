...
cooker_parallel_tasks() {
  local stage="$1"
  local raw_tasks="${COOKER_PARALLEL_TASKS:-}"

  if [[ -z "$raw_tasks" ]]; then
    raw_tasks="${FREEWAY_STAGE_PARALLEL_TASKS[$stage]:-}"
  fi
  if [[ -z "$raw_tasks" && -n "${SLURM_JOB_ID:-}" ]]; then
    raw_tasks="${SLURM_NTASKS:-}"
  fi
  if [[ -z "$raw_tasks" && -n "${SLURM_JOB_ID:-}" ]]; then
    raw_tasks="${recipe_slurm_ntasks:-1}"
  fi
  if [[ -z "$raw_tasks" ]]; then
    raw_tasks="1"
  fi

  cooker_positive_int_value "cooker parallel task count for $stage" "$raw_tasks"
}
